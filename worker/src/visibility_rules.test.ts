...
import { questionRow, visibilityRuleRow } from '../test/fixtures';
...
test('visibleQuestionIds evaluates choice rules with strict selected ids', () => {
  const visible = visibleQuestionIds(
    ...
    [visibilityRuleRow({ source_question_id: 1, target_question_id: 2, value_json: '5' })],
...
test('visibleQuestionIds rejects coerced selected choice ids', () => {
  assertBadRequest(
    () => visibleQuestionIds(
      ...
      [visibilityRuleRow({ source_question_id: 1, target_question_id: 2, value_json: '100' })],
...
test('visibleQuestionIds rejects non-string text rule values', () => {
  assertHttpError(
    () => visibleQuestionIds(
      ...
      [visibilityRuleRow({ source_question_id: 1, target_question_id: 2, value_json: '7' })],
